    ...
    <div className="space-y-4 sm:space-y-5 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <button
          ...
          className="px-3.5 sm:px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-xs font-bold flex items-center justify-center space-x-1.5 shadow-sm transition active:scale-95 touch-manipulation self-stretch sm:self-auto"
        >
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-slate-100 text-[11px] sm:text-xs">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 text-slate-500">
            ...
            <span>•</span>
            <span>Income: <strong className="text-emerald-600">{currency}{totalFilteredIncome.toLocaleString()}</strong></span>
          </div>
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((tx) => {
              ...
              return (
                <div 
                  key={tx.id}
                  className="p-3 sm:p-4 hover:bg-slate-50 transition flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 text-xs"
                >
                  ...
                  <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0 flex-1">
                    ...
                    <div className="space-y-0.5 min-w-0 flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {tx.isAutoImported && (
                          <span className="text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.2 rounded bg-slate-100 text-slate-600 shrink-0">
                            Auto
                          </span>
                        )}
                      </div>

                      <div className="flex flex-wrap items-center gap-1.5 text-[10px] sm:text-[11px] text-slate-500 font-medium">
                        <span>{tx.date}</span>
                        <span>•</span>
                        ...
                        {tx.merchant && (
                          <>
                            <span>•</span>
                            <span className="text-slate-700 truncate max-w-[120px]">{tx.merchant}</span>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      {isEditing ? (
                        <select
                          ...
                          className="text-xs bg-white text-slate-900 border border-emerald-500 rounded-lg px-2 py-1 focus:outline-none shadow-xs"
                        >
                          {ALL_CATEGORIES.map(c => (
                            <option key={c} value={c}>{c}</option>
                          ))}
                        ...
                        <button
                          ...
                          className="text-[10px] sm:text-[11px] font-semibold px-2 sm:px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition truncate max-w-[95px] sm:max-w-[130px] touch-manipulation"
